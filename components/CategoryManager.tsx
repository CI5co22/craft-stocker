...
export const CategoryManager: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  // Estado para controlar qué categorías están expandidas en el gestor
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col border border-slate-100 animate-in zoom-in-95 duration-200">
        ...
        <div className="p-5 flex-1 overflow-y-auto max-h-[70vh] space-y-6">
            <form onSubmit={handleAdd} className="flex gap-2">
                <input 
                    ...
                    className="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none text-slate-800 shadow-sm"
                    value={newCategory}
            ...
            <div className="space-y-4">
                {topLevel.length > 0 ? topLevel.map((cat) => {
                    ...
                    <div key={cat} className="space-y-2">
                        {/* Parent Category Row */}
                        <div className={`group flex items-center justify-between p-3 bg-slate-50 rounded-xl border transition-all ${editingCategory === cat ? 'border-emerald-500 ring-2 ring-emerald-100' : 'border-slate-100 hover:border-emerald-200'}`}>
                            {editingCategory === cat ? (
                                <div className="flex-1 flex items-center gap-1">
                                    <input 
                                        autoFocus
                                        className="flex-1 bg-white border border-emerald-300 rounded-lg px-2 py-1.5 text-xs outline-none focus:ring-2 focus:ring-emerald-500"
                                        value={editValue}
                                    ...
                                    />
                                    <button onClick={() => handleUpdate(cat)} className="p-1.5 text-emerald-600 hover:bg-emerald-100 rounded-lg transition-colors"><Check size={16} /></button>
                                    <button onClick={() => setEditingCategory(null)} className="p-1.5 text-slate-400 hover:bg-slate-100 rounded-lg transition-colors"><X size={16} /></button>
                                </div>
                            ) : (
                                <>
                                    <div className="flex items-center gap-2 flex-1 min-w-0" onClick={() => subCats.length > 0 && toggleExpand(cat)}>
                                        ...
                                        {subCats.length > 0 && !isExpanded && (
                                          <span className="text-[10px] text-slate-400 font-medium">({subCats.length})</span>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <button onClick={() => startEditing(cat)} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-white rounded-lg transition-colors shadow-sm"><Edit2 size={14} /></button>
                                        <button onClick={() => handleDeleteClick(cat)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-white rounded-lg transition-colors shadow-sm"><Trash2 size={14} /></button>
                                    </div>
                                </>
                            )}
                        </div>

                        {/* Subcategories (Children) - Solo si está expandido o se está añadiendo una */}
                        {(isExpanded || addingSubTo === cat) && (
                          <div className="ml-6 space-y-1.5 border-l-2 border-slate-100 pl-4 animate-scale-in">
                              {/* Input Inline para añadir subcategoría */}
                              {addingSubTo === cat && (
                                <div className="flex items-center gap-1 p-1 bg-emerald-50 border border-emerald-300 rounded-lg mb-2">
                                  <input 
                                    autoFocus
                                    className="flex-1 bg-white border-none text-[11px] px-2 py-1 outline-none rounded"
                                  ...
                                  />
                                  <button onClick={() => confirmAddSub(cat)} className="text-emerald-600 p-1 hover:bg-white rounded"><Check size={14} /></button>
                                  <button onClick={() => setAddingSubTo(null)} className="text-slate-400 p-1 hover:bg-white rounded"><X size={14} /></button>
                                </div>
                              )}

                              {subCats.map(sub => (
                                  <div key={sub} className={`group flex items-center justify-between p-2 bg-white rounded-lg border transition-all ${editingCategory === sub ? 'border-emerald-500' : 'border-slate-50 hover:border-emerald-100'}`}>
                                      {editingCategory === sub ? (
                                          <div className="flex-1 flex items-center gap-1">
                                              <input 
                                                  autoFocus
                                                  className="flex-1 border border-emerald-300 rounded px-2 py-1 text-[11px] outline-none"
                                                  value={editValue}
                                                  onChange={(e) => setEditValue(e.target.value)}
                                                  onKeyDown={(e) => {
                                              ...
                                              />
                                              <button onClick={() => handleUpdate(sub)} className="p-1 text-emerald-600"><Check size={14} /></button>
                                          ...
                                          <>
                                              <div className="flex items-center gap-2 flex-1 min-w-0">
                                                  ...
                                                  <span className="text-slate-500 font-medium text-xs truncate italic">{sub.split(' / ').pop()}</span>
                                              </div>
